...
{    
    ...
    for(ControllerZone* controller_zone: controller_zones)
    {
        ...
        if (ZT == ZONE_TYPE_SINGLE || ZT == ZONE_TYPE_LINEAR)
        {
            ...
            {
                controller_zone->SetLED(LedID, GetColor(1, reverse ? leds_count - LedID - 1 : LedID, leds_count, 1), Brightness, Temperature, Tint, Saturation);
            }
        }
        ...
        else if (ZT == ZONE_TYPE_MATRIX)
        {
            ...
            for (int col_id = 0; col_id < cols && col_id < colors_count; col_id++)
            {
                for (int row_id = 0; row_id < rows; row_id++)
                {
                    ...
                    controller_zone->SetLED(LedID, GetColor(reverse ? rows - row_id - 1: row_id, reverse ? cols - col_id - 1: col_id, rows, cols), Brightness, Temperature, Tint, Saturation);
                }
            }
        }
    }
